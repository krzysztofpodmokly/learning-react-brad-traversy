...
router.post('/', [
   ...
   try {
    ...
    if (user) {
        return res.status(400).json({ errors: [{ msg: "User already exists" }] }) // check Postman => Users & Auth collection => Register User Error
    }
    
    // 2. Get users gravatar
    const avatar = gravatar.url(email, { // Gravatar docs => https://github.com/emerleite/node-gravatar
        s: '200',
        r: 'pg',
        d: 'mm'
    });
